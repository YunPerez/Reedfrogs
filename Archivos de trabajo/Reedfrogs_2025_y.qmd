...
```{stan output.var="m_1", engine="stan", warning=FALSE, message=FALSE}
data {
  int<lower=0> T;         // Núm de tanques
  int<lower=0> S[T];         // Núm de renacuajos que sobrevivieron
...
model {
  alpha ~ normal(0, 1.5);    // Prior de aplha
...
generated quantities {
  int S_rep[T]; 
  // Predicciones basadas en probabilidad común
  for (t in 1:T) {
...
```{stan output.var="m_2", engine="stan", warning=FALSE, message=FALSE}
data {
...
model {
  for (i in 1:T) {
    alpha[i] ~ normal(0, 1.5);   // Priors para cada alpha_i
  }
...
```{stan output.var="m_3", engine="stan", warning=FALSE, message=FALSE}
data {
...
```{r datos_comparativos, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r graf_comparativo, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r int_cred, echo=FALSE, message=FALSE, warning=FALSE}
p3 <- ggplot(df, 
       aes(x = index, 
           y = propsurv_est3)) +
  geom_point(data=df, 
...
```{r diag_mcmc, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r pred_tanque, echo=FALSE, message=FALSE, warning=FALSE}
df_pred <- df |>
...
ggplot(df_pred, aes(x = index)) +
  ...
  scale_color_manual(values = c("Total pooling" = "steelblue",
...
```{r df_m4, echo=FALSE, message=FALSE, warning=FALSE}
df <- df |> 
...
```{stan output.var="m_4", engine="stan", warning=FALSE, message=FALSE}
data {
...
```{r ajuste_m4 , echo=FALSE, warning=FALSE, message=FALSE}
dat <- list(
  N = nrow(df),
  "T" = max(df$tank),
  surv = df$surv,
...
```{r tabla_m4, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r mod4_res, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r pred_m4, echo=FALSE, message=FALSE, warning=FALSE}
# 1) Extraer matrix [iter × tanques] de predicciones
s_rep4 <- rstan::extract(fit4, pars = "surv_rep")$surv_rep

# 2) Media de sobrevivientes estimados por tanque
df$urv_est4 <- apply(s_rep4, 2, mean)

# 3) Pasar a proporción de supervivencia
df$propsurv_est4 <- df$urv_est4 / df$density

# 4) Extraer el hiperparámetro global (mu_alpha) y transformarlo
mu4_post <- rstan::extract(fit4, pars = "mu_alpha")$mu_alpha
mu4_est  <- plogis(mean(mu4_post))
...
ggplot(df, aes(x = index)) +
  ...
  geom_point(aes(y = propsurv), color = "gray70") +
  # predicciones
  geom_point(aes(y = propsurv_est4), color = "green3") +
  # línea punteada de mu_alpha
  geom_hline(yintercept = mu4_est,
             linetype = "dashed",
             color    = "blue4") +
  # divisores por densidad
  geom_vline(xintercept = c(16, 32, 48), color = "cyan3") +
  # ejes y etiquetas
  scale_x_continuous(
    breaks = c(8, 24, 40),
  ...
  labs(
    x        = "Índice de tanque",
    y        = "Tasa de supervivencia",
    subtitle = "Gris: observados | Verde: Modelo 4"
  ) +
  theme_minimal(base_size = 14) +
  theme(
    axis.title = element_text(face = "italic")
...
```{r tabla_aux4, echo=FALSE, message=FALSE, warning=FALSE}
# 1) extrae la matriz de predicciones y conviértela a tibble
post4 <- rstan::extract(fit4, pars="surv_rep")$surv_rep
post4_df <- as_tibble(post4)

# 2) pivot, resumir y calcular proporciones
aux4 <- post4_df |>
  pivot_longer(
    cols      = everything(),
    names_to  = "parameter",
  ...
  group_by(parameter) |>
  summarise(
    m4_q2_5  = quantile(value, probs = 0.025),
    m4_mean  = mean(value),
    m4_q97_5 = quantile(value, probs = 0.975),
    .groups  = "drop"
  ) |>
  mutate(
    index = as.numeric(gsub("[^0-9]", "", parameter))
  ) |>
  left_join(
    df |> select(index, density),
    by = "index"
  ) |>
  mutate(
    m4_lower = m4_q2_5 / density,
    m4_sup   = m4_mean  / density,
    m4_upper = m4_q97_5 / density
  )
```

```{r comp_mod4, echo=FALSE, message=FALSE, warning=FALSE}
# Modelo jerárquico (partial pooling) con intervalos (usa aux3)
p3 <- ggplot(df, aes(x = index, y = propsurv_est3)) +
  geom_point(data = df, aes(y = propsurv), color = "grey") +
  geom_point(color = "blue3") +
  geom_segment(
    data = aux3,
    aes(x = index, xend = index,
        y = m3_lower, yend = m3_upper),
    color = "blue", alpha = 0.2, linewidth = 1
  ) +
  geom_hline(yintercept = alpha_est,
             linetype = "dashed", color = "blue4") +
  labs(
    x = "Tanque", y = "Tasa de supervivencia",
    title = "Modelo jerárquico (partial pooling)"
  ) +
  theme_minimal()

# Modelo completo
p2 <- ggplot(df, aes(x = index, y = propsurv_est4)) +
  geom_point(data = df, aes(y = propsurv), color = "grey") +
  geom_point(color = "green3") +
  geom_segment(
    data = aux4,
    aes(x = index, xend = index,
        y = m4_lower, yend = m4_upper),
    color = "green4", alpha = 0.2, linewidth = 1
  ) +
  geom_hline(yintercept = mu4_est,
             linetype = "dashed", color = "blue4") +
  labs(
...
# Los dos apilados verticalmente
p3 / p2
```
...
- Capturar la heterogeneidad real entre tanques. -Reducir la incertidumbre en tanques pequeños mediante shrinkage adaptativo. /
