...
1.  **Reproducir y comprender** el ejemplo de *Statistical Rethinking* aplicado a los datos de Reedfrogs.
...
3.  **Explorar** el trade-off underfitting/overfitting comparando los tres niveles de pooling (*pooling*, *no pooling* y *partial pooling*) mediante criterios de información y gráficos de predicción.
4.  **Desplegar** un análisis causal formal con un DAG que recoja nuestros supuestos de identificación.
...
```{r grafica_reedfrogs, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r dag, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="DAG causal: densidad, tamaño, depredadores y jerarquía de tanque"}
grViz("
digraph {
...
```{r odds_prob, echo=FALSE, warning=FALSE, message=FALSE}
...
```{r auxgraf_comp_mod, echo=FALSE, warning=FALSE, warning=FALSE}
...
```{r graf_comp_mod, echo=FALSE, warning=FALSE, warning=FALSE}
...
```{r graf_int_cred, echo=FALSE, warning=FALSE, message=FALSE}
p2 <- ggplot(df, 
       aes(x = index, 
           y = propsurv_est2)) +
  geom_point(data=df, 
...
p3 <- ggplot(df, 
  ...
  labs(x = "Tanque", 
       y = "",
       title="Modelo Parcial pPoling - Jerárquico") +
  theme(
    axis.title.x = element_text(face = "bold"),
    axis.title.y = element_text(face = "bold")
  ) +
...
```{r trazas_dist_post, echo=FALSE, warning=FALSE, message=FALSE}
...
```{r df_mod_comp, echo=FALSE, message=FALSE, warning=FALSE}
df <- df |> 
...
```{r ajuste_m4, echo=FALSE, message=FALSE, warning=FALSE}
dat <- list(
  N = nrow(df),
  "T" = max(df$tank),
  surv = df$surv,
...
```{r hiper_mod_comp, echo=FALSE, warning=FALSE, message=FALSE}
...
```{r graf_m4_comp, echo=FALSE, message=FALSE, warning=FALSE}
p5<- ggplot(df, 
       aes(x = index, 
           y = propsurv_est3)) +
  geom_point(data=df, 
